...
export class ImpactStoryService {
  ...
  getStoryById(storyId: number): Observable<ImpactStoryDTO> {
    ...
    return this.apiService.get<ImpactStoryDTO>(`/public/impact-stories/${storyId}`).pipe(
      tap(story => {
  ...
  getStoriesByCampaign(campaignId: number): Observable<ImpactStoryDTO[]> {
    this.loading.set(true);
    return this.apiService.get<ImpactStoryDTO[]>(`/public/campaigns/${campaignId}/impact-stories`).pipe(
      tap(stories => {
  ...
  getImpactStoriesByDonor(donorId: number): Observable<ImpactStoryDTO[]> {
    this.loading.set(true);
    return this.apiService.get<ImpactStoryDTO[]>(`/donors/${donorId}/impact-stories`).pipe(
      tap(stories => {
        this.stories.set(stories);
        this.loading.set(false);
      })
  ...
  // Admin endpoints
  createImpactStory(adminId: number, campaignId: number, story: CreateImpactStoryRequest): Observable<ImpactStoryDTO> {
    ...
    return this.apiService.postFormData<ImpactStoryDTO>(`/admins/${adminId}/impact-stories/campaign/${campaignId}`, formData);
  }
}
